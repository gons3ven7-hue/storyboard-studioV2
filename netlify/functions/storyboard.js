...
exports.handler = async function (event) {
  ...
  let perfil, roteiro, contexto, maxCenas;
  try {
    const b = JSON.parse(event.body);
    perfil = b.perfil; roteiro = b.roteiro; contexto = b.contexto;
    maxCenas = Math.min(4, Math.max(1, Number(b.maxCenas) || 3));
  ...
  const prompt =
    ...
    "Gere no MÁXIMO " + maxCenas + " cenas para este trecho. Seja detalhado, mas objetivo.\n\n" +
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${MODELO}:generateContent?key=${GEMINI_KEY}`,
      {
        ...
        body: JSON.stringify({
          ...
          generationConfig: {
            ...
            // Ajustado para caber no limite de 10s de execução do Netlify.
            // O app divide o roteiro em blocos e chama várias vezes.
